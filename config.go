...
func (self *Config) GetConfigBool(key string) bool {
	str := self.GetConfigStr(key)
...
func (self *Config) GetConfigInt64(key string) int64 {
	return int64(self.GetConfigInt(key))
}
func (self *Config) GetConfigUint64(key string) uint64 {
	return uint64(self.GetConfigInt(key))
}
		...
		case xml.StartElement:
			...
			case mynode == true:
				...
				for _, v := range value.Attr {
					...
					if v.Name.Local == "ip" {
						if ips, err := net.LookupIP(v.Value); err == nil && len(ips) > 0 {
							v.Value = ips[0].String()
						}
					}
					if list == true {
...
func GetConfigInt64(key string) int64 {
	return global.GetConfigInt64(key)
}
func GetConfigUint64(key string) uint64 {
	return global.GetConfigUint64(key)
}
func GetConfigBool(key string) bool {
	return global.GetConfigBool(key)
}
